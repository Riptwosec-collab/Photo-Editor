...
export async function encodeStored(value: unknown): Promise<unknown> {
 if (value instanceof Blob) return {__lumaforgeBinary:1,type:value.type,bytes:await value.arrayBuffer()};
...
export function decodeStored<T>(value: unknown): T {
 ...
 if (value && typeof value === "object" && Object.getPrototypeOf(value) === Object.prototype) {
  ...
  if(row.__lumaforgeBinary===1 && row.bytes instanceof ArrayBuffer && typeof row.type === "string") return new Blob([row.bytes],{type:row.type}) as T;
