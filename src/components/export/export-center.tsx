...
export function ExportCenter() {
  ...
  const [preflight,setPreflight]=useState<{key:string;notes:string[]}>({key:"",notes:[]});
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void prepareExport(target, project, options, controller.signal,notes=>{if(!controller.signal.aborted)setPreflight({key,notes});}).then(async (blob) => {
  ...
  async function download() {
    ...
    try {
      ...
      await saveExportRecord({ id: crypto.randomUUID(), projectId: project.id, createdAt: new Date().toISOString(), format: options.format, quality: options.quality, longEdge: options.longEdge, width: ready.width, height: ready.height, filename, colorSpace: options.colorSpace === "display-p3" ? "Display P3" : "sRGB" });
