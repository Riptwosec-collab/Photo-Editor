...
export function BatchWorkspace() {
  ...
  function addFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const valid = files.filter((f) => ["image/jpeg","image/png","image/webp"].includes(f.type) && f.size <= 30*1048576);
    ...
    setMessage(valid.length !== files.length ? "Some files were skipped: supported types are JPG, PNG and WebP up to 30 MB each." : "Images added to the saved queue"); event.target.value = "";
  }
  async function run(errorsOnly = false) {
    ...
    for (const row of queue) {
      ...
      try {
        ...
        const project: StoredProject = { id: row.id, name: row.file.name, imageName: row.file.name, imageType: row.file.type, imageBlob: row.file, width: 1, height: 1, createdAt: "", updatedAt: "", adjustments: { ...DEFAULT_ADJUSTMENTS, ...recipe }, geometry: DEFAULT_GEOMETRY };
