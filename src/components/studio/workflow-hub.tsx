...
export function WorkflowHub({ portrait = false }: { portrait?: boolean }) {
  ...
  return <AppShell><main className="studio-home"><span className="studio-kicker">{portrait ? "NATURAL PORTRAITS" : "ASSISTED CREATIVITY"}</span><div className="studio-heading"><h1>{portrait ? "Beauty Studio" : "AI Studio"}</h1><Link className="button primary" href="/editor">เปิดภาพ <ArrowUpRight size={16} /></Link></div><p className="hub-intro">{portrait ? "ให้แสงและสีช่วยเล่าเรื่องของคนในภาพ" : "จากไอเดีย สู่ภาพที่คุณต้องการ"}</p><div className="studio-workflows hub-tools">{tools.map(({ title, description, tool, href, icon: Icon }) => <Link className="studio-workflow" href={href ?? `/editor?tool=${tool}`} key={title}><span className="workflow-icon"><Icon size={23} /></span><ArrowUpRight size={17} className="workflow-arrow" /><h3>{title}</h3><p>{description}</p></Link>)}</div><section className="shortcut-card"><h2>{portrait ? "เกี่ยวกับการปรับภาพบุคคล" : "Local tools · No cloud model connected"}</h2><p>{portrait ? "เครื่องมือปรับแสงและสีมีผลกับภาพทั้งภาพ การแยกใบหน้า รีทัชเฉพาะผิว และลบวัตถุด้วย AI ยังไม่พร้อมใช้งาน" : "การวิเคราะห์แสงใช้สถิติพิกเซล ส่วนแผนจากข้อความใช้กฎและสูตรแต่งภาพ ไม่ใช่โมเดลสร้างภาพหรือระบบจดจำวัตถุ ทุกเครื่องมือเปิดใน Editor เดียวกันเพื่อใช้ประวัติ Undo และการส่งออกร่วมกัน"}</p></section></main></AppShell>;
}
