...
async function signature() {
  ...
  return JSON.stringify({ source: [...new Uint8Array(digest)].map((n) => n.toString(16).padStart(2,"0")).join(""), recipe: s.committed });
}
export function CloudAiPanel() {
  ...
  return <section className="cloud-ai-panel"><h3> <T text={"AI image tools"} /> </h3><label> <T text={"Tool"} /> <select value={kind} disabled={busy} onChange={(e) => { setKind(e.target.value); setConsent(false); }}><option value="background">Remove background</option><option value="inpaint">Remove object / Inpaint</option></select></label>{kind === "inpaint" && <><label> <T text={"Replacement prompt"} /> <textarea maxLength={500} value={prompt} onChange={(e) => setPrompt(e.target.value)} /></label><button disabled={!image || useEditorStore.getState().layers.length >= 20} onClick={() => { const s=useEditorStore.getState(); const layer=createLayer(); layer.name="Object removal mask"; layer.adjustments={}; layer.mask.kind="brush"; s.addLayer(layer); s.setMaskEditing(true); const ui=useStudioStore.getState(); ui.setCompareMode("off"); ui.setInspectorCollapsed(false); ui.setActiveInspectorSection("layers"); }}> <T text={"Paint object mask"} /> </button></>}<p>{capabilities[kind]?.enabled ? `Provider estimate: ${capabilities[kind].cost}` : "Provider setup required. These tools are not active yet."}</p><label className="consent-row"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} /> <T text={"Send this image and mask to Replicate. Charges follow the configured model pricing."} /> </label><small> <T text={"AI working image: up to 1536 px. Your original is kept. Review the result before adding it as a layer."} /> </small><button disabled={!image || !consent || !capabilities[kind]?.enabled || busy || Boolean(job && ["starting","processing"].includes(job.status))} onClick={() => void start()}><T text={busy ? "Uploading…" : "Start AI edit"} /></button>{job && <div className="ai-job-card"><p role="status"> <T text={"Job:"} /> {job.status}</p><button onClick={() => void check(job).catch((e) => setMessage(e.message))}> <T text={"Refresh status"} /> </button>{["starting","processing"].includes(job.status) && <button onClick={async () => { try { const r=await fetch("/api/ai/jobs",{ method:"POST",headers:{...await auth(),"Content-Type":"application/json"},body:JSON.stringify({action:"cancel",token:job.token}) }); const b=await r.json(); if(!r.ok) throw new Error(b.error); save({...job,status:b.status}); } catch(e) {setMessage(e instanceof Error?e.message:"Cancel failed");} }}> <T text={"Cancel job"} /> </button>}{job.result && <><Image src={job.result} width={400} height={300} alt="AI result awaiting review" unoptimized /><button disabled={!image || useEditorStore.getState().layers.length >= 20} onClick={async () => { try { if(await signature()!==job.signature) {setMessage("The image changed after this request. Restore the earlier recipe or start a new edit.");return;} const layer=createLayer("raster"); layer.dataUrl=job.result; layer.name=job.kind === "background" ? "AI background cutout" : "AI object removal"; useEditorStore.getState().addLayer(layer); setMessage("Result added as a reversible layer"); } catch(e) { setMessage(e instanceof Error ? e.message : "Could not add result"); } }}> <T text={"Add result as layer"} /> </button></>}</div>}{message && <p role="alert"><T text={message} /></p>}</section>;
}
