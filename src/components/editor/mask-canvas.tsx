...
export function MaskCanvas() {
  ...
  return <><span role="status" className="sr-only">{error}</span><canvas ref={canvas} width={1000} height={1000} className="mask-paint-canvas" aria-label="Paint layer mask" onPointerDown={(e) => {
    ...
    if (layer.retouch) { const p=point(e); useEditorStore.getState().updateLayer(layer.id,{retouch:[...layer.retouch,{...p,radius:Math.min(.2,radius),sourceX:layer.x??.45,sourceY:layer.y??.45}].slice(-300)}); return; }
