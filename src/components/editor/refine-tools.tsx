...
export function RefineTools({layer}:{layer:EditorLayer}) {
 ...
 <button onClick={async()=>{try{const mask=document.createElement("canvas");const image=useEditorStore.getState().image;mask.width=1000;mask.height=Math.max(1,Math.round(1000*(image?.height??1)/(image?.width??1)));await drawMask(mask,layer.mask);update({mask:{...layer.mask,kind:"brush",dataUrl:mask.toDataURL("image/png"),strokes:[],invert:false,feather:0,edgeShift:0,edgeContrast:0}});mask.width=mask.height=0;useEditorStore.getState().setMaskEditing(true);useStudioStore.getState().setCompareMode("off");}catch{setError("Could not draw mask");}}}><T text="Refine with brush"/></button>
