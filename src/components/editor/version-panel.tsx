...
export function VersionPanel({ projectId }: { projectId: string | null }) {
  ...
  return (
    <div className="panel-scroll version-panel">
      <div className="panel-title">
        <div><span className="kicker">Non-destructive</span><h2>Snapshots</h2></div>
        <button className="icon-button" title="Refresh snapshots" onClick={() => void refresh()}><RotateCcw size={16} /></button>
      </div>
      <p className="control-note">Snapshots store edit recipes and geometry. The original image remains in the project.</p>
      <button className="button primary compact" disabled={!projectId} onClick={() => void createSnapshot()}><Camera size={16} /> Create snapshot</button>
      ...
      <div className="version-list">
        {versions.map((version) => (
          <article key={version.id} className="version-row">
            <button className="version-restore" onClick={() => { loadRecipe(version.adjustments, version.geometry); setStatus(`Restored ${version.name}`); }}>
              <strong>{version.name}</strong>
              <small>{new Date(version.createdAt).toLocaleString()}</small>
            </button>
            <button className="icon-button" title="Rename snapshot" onClick={() => void editName(version)}><Pencil size={14} /></button>
            <button className="icon-button danger-button" title="Delete snapshot" onClick={() => void remove(version)}><Trash2 size={14} /></button>
          </article>
        ))}
      </div>
