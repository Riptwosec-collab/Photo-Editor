...
export default function SettingsPage() {
  ...
  ].map(({ key, icon: Icon, title, detail }) => <label className="setting-row" key={key}><Icon size={22} /><span><strong><T text={title} /></strong><small><T text={detail} /></small></span><input type="checkbox" role="switch" aria-label={title} checked={preferences[key]} onChange={(event) => preferences.setPreference(key, event.target.checked)} /></label>)}</section><section className="setting-row storage-summary"><HardDrive size={22} /><span><strong> <T text={"พื้นที่เบราว์เซอร์ที่ใช้ / โควตาประมาณ"} /> </strong><small>{storage}</small><small> <T text={"เป็นพื้นที่รวมของเว็บไซต์บนอุปกรณ์นี้ เก็บไฟล์ส่งออกสำรองไว้ก่อนล้างข้อมูลเบราว์เซอร์"} /> </small></span></section><section className="shortcut-card"><h2> <T text={"Keyboard shortcuts"} /> </h2><p><kbd>Ctrl / ⌘ K</kbd> <T text={"ค้นหาเมนู"} /> <kbd>Ctrl / ⌘ S</kbd> <T text={"บันทึก"} /> <kbd>Ctrl / ⌘ Z</kbd> <T text={"ย้อนกลับ"} /> <kbd>\</kbd> <T text={"ดูต้นฉบับ"} /> </p></section></main></AppShell>;
}
