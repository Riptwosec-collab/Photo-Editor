...
export function createLocalEditPlan(prompt:string):AiEditPlan{
  const text=prompt.toLowerCase(); const changes:AiEditPlan["changes"]=[]; const detected=["Natural-language editing intent"];
  ...
  if(/night|กลางคืน|noise|นอยส์/.test(text)){add(changes,"shadows",24,"Recover low-light detail");add(changes,"noiseReduction",22,"Apply the local noise reduction filter");add(changes,"highlights",-30,"Control point-light clipping");}
  ...
  return {provider:"local-rule-based-demo",summary:"A transparent local rule-based edit plan was generated. No cloud model processed the image.",detected,changes:unique,warnings:["DEMO provider: recommendations are prompt rules, not computer-vision analysis."]};
}
