...
export const layersSchema = z.array(z.object({
  ...
  retouch: z.array(z.object({x:z.number().min(0).max(1),y:z.number().min(0).max(1),radius:z.number().min(.001).max(.2),sourceX:z.number().min(0).max(1),sourceY:z.number().min(0).max(1)})).max(300).optional(),
